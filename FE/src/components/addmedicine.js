...
function AddMedicine() {
  ...
  const handleUpperCase = (event) => {
    ...
    // Update the state or do something with the uppercaseValue
    setFormData({
      ...formData,
      [id]: uppercaseValue,
    });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.post("http://localhost:4000/purchase", formData);
                    ...
                    <div className="input-group">
                      ...
                      <datalist id="unitOptions">
                        ...
                        {formData.dosageUnit && !["mg", "ml", "gm"].includes(formData.dosageUnit.toUpperCase()) && (
                          <option value={formData.dosageUnit} />
                        )}
                      </datalist>
                    </div>
                  </div>
